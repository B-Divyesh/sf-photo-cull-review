...
export function selectMediaFiles(incoming: File[], paid: boolean): { media: File[]; skipped: number } {
  ...
  if (!paid && media.length > FREE_FILE_LIMIT) {
    throw new Error(`This folder contains ${media.length.toLocaleString()} supported files. The free product scans up to ${FREE_FILE_LIMIT}; choose a smaller folder or buy an Archive pass.`);
  }
